          ...
          "bg-grid": (value: string) => {
            if (/var\(--/gi.test(value)) {
              const color = value.replace(/var\(--color-([\w-]+\d+)\)/gi, (_, match) => {
                const colorKey = match.split("-")[0].replace(/^gray$/gi, "trueGray") as ColorKey;
                const colorIndex = +match.split("-")[1] as unknown as keyof typeof colors['slate'];
                const colorValue = colors[colorKey][colorIndex];
